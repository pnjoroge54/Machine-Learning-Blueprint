...
def estimate_standardized_ou_parameters(price_series, moving_avg_series, vol_series):
    ...
    """
    data = pd.DataFrame(
        {"price": price_series, "mu": moving_avg_series, "vol": vol_series}
    ).dropna()
...
def run_triple_barrier_simulation(
    ...
    for pt in pt_levels:
        for sl in sl_levels:
            ...
            profit_factor = (
                (
                    np.mean([pnl for pnl in pnl_list if pnl > 0])
                ...
                if any(pnl < 0 for pnl in pnl_list)
                else float("inf")
            )
...
def example_usage():
    ...
    for _ in range(n_periods):
        next_price = (
            (1 - true_phi) * true_mu + true_phi * prices[-1] + true_sigma * gauss(0, 1)
        )
