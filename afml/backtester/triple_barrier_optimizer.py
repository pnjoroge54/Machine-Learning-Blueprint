...
class OptimalBarriers:
    ...
    profit_taking_multiple: float  # Optimal profit-taking barrier (π̄)
    stop_loss_multiple: float  # Optimal stop-loss barrier (π)
...
@njit
def simulate_ou_path(phi: float, forecast: float, sigma: float, 
                     seed: float, max_hp: int) -> Tuple[np.ndarray, int]:
    """
...
@njit
def check_barrier_hit(path: np.ndarray, profit_taking: float, 
                     stop_loss: float, max_hp: int, seed: float) -> Tuple[float, int]:
    """
...
def optimize_barriers(
    ...
    if verbose:
        ...
        print(f"O-U Parameters: φ={ou_params.phi:.4f}, σ={ou_params.sigma:.4f}, "
              f"forecast={ou_params.forecast:.4f}, half-life={ou_params.half_life:.2f}")
    
    count = 0
    for pt, sl in product(pt_scaled, sl_scaled):
        count += 1
        ...
        if NUMBA_AVAILABLE:
            ...
            for _ in range(n_iter):
                path, _ = simulate_ou_path(ou_params.phi, ou_params.forecast, 
                                          ou_params.sigma, seed, max_holding_period)
    ...
    optimal = OptimalBarriers(
        ...
        profit_taking_multiple=optimal_row['profit_taking']/ou_params.sigma,
        stop_loss_multiple=optimal_row['stop_loss']/ou_params.sigma,
    ...
    if verbose:
        ...
        print("="*70)
        print(f"Profit Taking: {optimal.profit_taking:.4f} "
              f"({optimal.profit_taking/ou_params.sigma:.2f}σ)")
        print(f"Stop Loss:     {optimal.stop_loss:.4f} "
              f"({optimal.stop_loss/ou_params.sigma:.2f}σ)")
...
def find_optimal_stop_loss_given_target(
    ...
    for sl in stop_loss_range:
        returns, _ = simulate_barrier_outcomes(
            ou_params.phi, ou_params.forecast, ou_params.sigma,
            seed, profit_target, sl, n_iter, max_holding_period
        )
...
def find_optimal_profit_taking_given_max_loss(
    ...
    for pt in profit_taking_range:
        ...
        returns, _ = simulate_barrier_outcomes(
            ou_params.phi, ou_params.forecast, ou_params.sigma,
            seed, pt, max_stop_loss, n_iter, max_holding_period
        )
...
def generate_optimal_label_config(
        symbol, train_start, train_end, data_config, strategy, forecast_window, max_holding_period, pt_range, sl_range, min_ret=0.00005, n_iter=100000, vertical_barrier_zero=True, filter_as_series=True,
        ):
    ...
    data = load_and_prepare_training_data(
        ...
        end_date=train_end,
        account_name=data_config["account_name"],
        bar_type=data_config["bar_type"],
        bar_size=data_config["bar_size"],
        price=data_config["price"],
        )
    results = optimize_strategy_barriers(
        strategy, data, forecast_window=forecast_window, n_iter=n_iter, max_holding_period=max_holding_period, pt_range=pt_range, sl_range=sl_range,
        )
    optimal_barriers = results.optimal

    label_config = dict(
        target_lookback=forecast_window,
        profit_target=optimal_barriers.profit_taking_multiple,
        stop_loss=optimal_barriers.stop_loss_multiple,
...
if __name__ == "__main__":
    # Example: Generate synthetic data and test
    np.random.seed(42)
    
    dates = pd.date_range('2020-01-01', periods=1000, freq='D')
    price = 100 * np.exp(np.cumsum(np.random.normal(0.001, 0.02, 1000)))
    ...
    print("="*70)
    print("TRIPLE BARRIER OPTIMIZATION EXAMPLE")
    print("="*70)
    
    # Note: This requires the BaseStrategy classes from your module
    # from trading_strategies import BollingerStrategy
    # strategy = BollingerStrategy(window=20, std=2.0)
    # result = optimize_strategy_barriers(strategy, data, verbose=True)
    
    print("\nTo use with your strategies:")
    print("  from trading_strategies import BollingerStrategy, MACrossoverStrategy")
    print("  strategy = BollingerStrategy(window=20, std=2.0)")
    print("  result = optimize_strategy_barriers(strategy, data)")
...
def plot_barrier_heatmap(
    ...
    pt_vals = df['pt_sigma_multiple'].unique()
    sl_vals = df['sl_sigma_multiple'].unique()
    pt_idx = np.argmin(np.abs(pt_vals - opt_pt_mult))
    sl_idx = np.argmin(np.abs(sl_vals - opt_sl_mult))
    
    ax.plot(pt_idx + 0.5, sl_idx + 0.5, 'r*', markersize=20, 
...
def plot_barrier_comparison(
    results: List[BarrierOptimizationResult],
    ...
    metrics = ['sharpe_ratio', 'win_rate', 'mean_return', 
               'std_return', 'avg_holding_period']
    metric_labels = ['Sharpe Ratio', 'Win Rate', 'Mean Return', 
                     'Std Return', 'Avg Holding Period']
    ...
    for idx, (metric, label) in enumerate(zip(metrics, metric_labels)):
        ...
        bars = ax.bar(range(n_strategies), values, alpha=0.7, 
    ...
    plt.suptitle('Strategy Comparison - Optimal Barrier Performance', 
...
def plot_ou_parameter_distribution(
    ...
    for _ in range(n_paths):
        path, _ = simulate_ou_path(ou.phi, ou.forecast, ou.sigma, 0.0, n_steps)
        paths.append(path)
    ...
    # Plot barriers
    ax1.axhline(opt.profit_taking, color='green', linestyle='--', linewidth=2, 
                label=f'Profit Taking ({opt.profit_taking:.3f})')
    ax1.axhline(-opt.stop_loss, color='red', linestyle='--', linewidth=2, 
                label=f'Stop Loss ({-opt.stop_loss:.3f})')
    ax1.axhline(ou.forecast, color='black', linestyle=':', linewidth=1.5, 
                label=f'Forecast ({ou.forecast:.3f})')
    
    ax1.set_xlabel('Time Steps', fontsize=11)
    ax1.set_ylabel('Price Return', fontsize=11)
    ax1.set_title(f'O-U Process Sample Paths\nφ={ou.phi:.3f}, σ={ou.sigma:.3f}, '
                  f'Half-life={ou.half_life:.1f}', fontweight='bold')
    ...
    returns, holding_periods = simulate_barrier_outcomes(
        ou.phi, ou.forecast, ou.sigma, 0.0,
        opt.profit_taking, opt.stop_loss, 10000, 100
    )
    
    # Plot distribution
    ax2.hist(returns, bins=50, alpha=0.7, color='steelblue', edgecolor='black')
    ax2.axvline(opt.profit_taking, color='green', linestyle='--', linewidth=2, 
                label='Profit Taking')
    ax2.axvline(-opt.stop_loss, color='red', linestyle='--', linewidth=2, 
                label='Stop Loss')
    ax2.axvline(np.mean(returns), color='orange', linestyle='-', linewidth=2, 
                label=f'Mean Return ({np.mean(returns):.4f})')
    
    ax2.set_xlabel('Exit Return', fontsize=11)
    ax2.set_ylabel('Frequency', fontsize=11)
    ax2.set_title(f'Exit Return Distribution (10k simulations)\n'
                  f'Win Rate: {opt.win_rate:.1%}, Sharpe: {opt.sharpe_ratio:.3f}', 
                  fontweight='bold')
    ax2.legend(loc='best', fontsize=9)
    ax2.grid(axis='y', alpha=0.3)
    
    plt.suptitle(f'{result.strategy_name} - O-U Process Analysis', 
                 fontsize=14, fontweight='bold')
    plt.tight_layout()
...
def create_full_report(
    ...
    print("\nOptimal Barriers:")
    print(f"  Profit Taking:  {result.optimal.profit_taking:.4f} "
          f"({result.optimal.profit_taking/result.ou_params.sigma:.2f}σ)")
    print(f"  Stop Loss:      {result.optimal.stop_loss:.4f} "
    ...
    if save_dir:
        ...
        heatmap_path = os.path.join(save_dir, f"{result.strategy_name}_heatmap.png")
        ou_path = os.path.join(save_dir, f"{result.strategy_name}_ou_analysis.png")
    else:
        heatmap_path = None
        ou_path = None
    
    # Plot 1: Heatmap
    plot_barrier_heatmap(result, save_path=heatmap_path)
    
    # Plot 2: O-U Analysis
    plot_ou_parameter_distribution(result, save_path=ou_path)
    
    # Save grid results
    if save_dir:
        csv_path = os.path.join(save_dir, f"{result.strategy_name}_grid_results.csv")
