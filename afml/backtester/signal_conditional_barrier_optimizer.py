...
class SignalConditionalBarrierOptimizer:
    ...
    def __init__(self, price_series, signal_series, returns_series=None):
        ...
        self.returns = (
            returns_series if returns_series is not None else price_series.pct_change()
        )

        # Align all series
        self.data = pd.DataFrame(
            {"price": self.prices, "signal": self.signals, "returns": self.returns}
        ).dropna()
        ...
        print(
            f"Signal periods: {len(self.signal_periods)} ({len(self.signal_periods) / len(self.data) * 100:.1f}%)"
        )
    ...
    def estimate_volatility_methods(self, lookback_windows=[20, 50, 100]):
        ...
        for window in lookback_windows:
            ...
            vol_methods[f"rolling_std_{window}"] = self.data["returns"].rolling(
                window=window
            ).std() * np.sqrt(252)

            # Method 2: EWMA volatility
            vol_methods[f"ewma_{window}"] = self.data["returns"].ewm(
                span=window
            ).std() * np.sqrt(252)
    ...
    def _estimate_garch_volatility(self, window=100):
        ...
        for i in range(window, len(returns)):
            ...
            if i == window:
                ...
                prev_vol = (
                    garch_vol.iloc[i - 1]
                    if not pd.isna(garch_vol.iloc[i - 1])
                    else initial_vol
                )
    ...
    def estimate_signal_conditional_ou_parameters(
        self, volatility_method="rolling_std_20", separate_by_signal_type=True
    ):
        """
        ...
        if volatility_method not in vol_methods:
            print(
                f"Volatility method {volatility_method} not found. Using rolling_std_20"
            )
        ...
        self.signal_periods["volatility"] = optimal_vol.reindex(
            self.signal_periods.index
        )
        ...
        if separate_by_signal_type:
            ...
            for signal_type in signal_types:
                ...
                if len(signal_subset) < 20:
                    print(
                        f"Warning: Only {len(signal_subset)} observations for signal type {signal_type}"
                    )
                    continue

                # Calculate moving average for this signal type
                signal_subset["moving_avg"] = (
                    signal_subset["price"].rolling(window=20).mean()
                )
            ...
            self.signal_periods["moving_avg"] = (
                self.signal_periods["price"].rolling(window=20).mean()
            )
            clean_data = self.signal_periods.dropna()

            params = self._estimate_ou_params_with_vol(
                clean_data["price"], clean_data["moving_avg"], clean_data["volatility"]
            )
    ...
    def _estimate_ou_params_with_vol(self, price_series, ma_series, vol_series):
        ...
        """
        data = pd.DataFrame(
            {"price": price_series, "ma": ma_series, "vol": vol_series}
        ).dropna()
    ...
    def optimize_barriers_by_signal_type(
        self,
        ou_params,
        vol_series,
        nIter=10000,
        ...
        for signal_type, params in ou_params.items():
            ...
            if "signal_" in signal_type:
                ...
                forward_returns = signal_subset["returns"].shift(
                    -1
                )  # Next period return
            ...
            with ProcessPoolExecutor(max_workers=mp.cpu_count()) as executor:
                ...
                for future, pt, sl in futures:
                    try:
                        ...
                        result.update(
                            {"pt_level": pt, "sl_level": sl, "signal_type": signal_type}
                        )
    ...
    def _simulate_single_combo_numba(self, phi, sigma, drift, maxHP, pt, sl, nIter):
        ...
        if not NUMBA_AVAILABLE:
            return self._simulate_single_combo_python(
                phi, sigma, drift, maxHP, pt, sl, nIter
            )
...
def demonstrate_signal_conditional_optimization():
    ...
    price_series = pd.Series(
        prices[1:], index=pd.date_range("2022-01-01", periods=n_days)
    )
    ...
    ou_params, optimal_vol = optimizer.estimate_signal_conditional_ou_parameters(
        volatility_method=best_vol_method, separate_by_signal_type=True
    )
