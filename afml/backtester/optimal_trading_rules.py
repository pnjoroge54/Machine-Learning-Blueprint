...
def simulate_rgp_path(
    ...
    for t in range(1, steps + 1):
        ...
        # Jump component
        if (
            params.jump_lambda is not None
            and params.jump_lambda > 0
            and rng.random() < params.jump_lambda
        ):
...
def simulate_multiple_paths(
    ...
    for i in range(n_paths):
        paths[i] = simulate_rgp_path(
            start_price, steps, params, random_state=rng.integers(0, 2**32)
        )
...
@njit(cache=True)
def simulate_single_path_numba(
    start_log_price: float,
    steps: int,
    mu: float,
    sigma: float,
    phi: float,
...
def simulate_multiple_paths_numba(
    start_price: float,
    steps: int,
    params: RGPParams,
    n_paths: int = 1000,
    ...
    use_jumps = (
        params.jump_lambda is not None
        and params.jump_lambda > 0
        and params.jump_mu is not None
    )
...
def simulate_all_paths_numba_parallel(
    start_log_price: float,
    steps: int,
    mu: float,
    sigma: float,
...
def simulate_multiple_paths_ultra(
    start_price: float, steps: int, params: RGPParams, n_paths: int = 1000
) -> np.ndarray:
...
def calculate_volatility(
    price_series: pd.Series, lookback_period: int = 20, method: str = "returns"
) -> pd.Series:
    """
...
def validate_rule_oos_volatility(
    ...
    with mlflow.start_run():
        ...
        logger.info(
            f"Volatility stats - Mean: {volatility_arr.mean():.6f}, "
        ...
        if len(valid_entries) < len(entry_idx_all):
            logger.warning(
                f"Removed {len(entry_idx_all) - len(valid_entries)} entries due to insufficient volatility data"
            )
        ...
        for fold_idx, (train_indices, test_indices) in enumerate(splits):
            with mlflow.start_run(nested=True, run_name=f"fold_{fold_idx}"):
                ...
                logger.info(
                    f"Fold {fold_idx} - Best PT multiple: {best_pt_multiple:.2f}, "
                ...
                trades = []
                run_trade_path_fn = (
                    run_trade_path_on_series_volatility
                    if sides is None
                    else run_trade_path_with_direction
                )
                    ...
                    trades.append(
                        {
                            ...
                            "pt_level": price_arr[entry_idx]
                            * (1 + best_pt_multiple * entry_vol),
                            "sl_level": price_arr[entry_idx]
                            * (1 - best_sl_multiple * entry_vol),
                            "return": float(ret),
                            "holding": int(holding),
                            "exit_reason": reason,
                            "fold": fold_idx,
                ...
                logger.success(
                    f"Fold {fold_idx} completed: {len(trades_df)} trades, "
        ...
        logger.success(
            f"Validation completed: {len(all_trades_df)} total trades, "
...
def calculate_trading_metrics(trades_df: pd.DataFrame) -> Dict:
    ...
    avg_pt_multiple = (
        trades_df["pt_multiple"].mean()
        if "pt_multiple" in trades_df.columns
        else float("nan")
    )
    avg_sl_multiple = (
        trades_df["sl_multiple"].mean()
        if "sl_multiple" in trades_df.columns
        else float("nan")
    )
...
def volatility_rule_search_fn(
    ...
    """
    logger.info(
        f"Searching for optimal volatility multiples on {len(train_entries)} training entries"
    )

    # Define parameter grid for volatility multiples
    if pt_sl_grid is None:
        pt_multiples = sl_multiples = np.linspace(
            0.5, 10.0, 20
        )  # PT: 0.5x to 10.0x volatility
    else:
    ...
    for pt in pt_multiples:
        for sl in sl_multiples:
            ...
            if len(fold_returns) > 1:
                sharpe = (
                    np.mean(fold_returns) / np.std(fold_returns)
    ...
    logger.info(
        f"Optimal multiples found: PT={best_pt:.2f}x vol, SL={best_sl:.2f}x vol, Sharpe={best_sharpe:.4f}"
    )
...
def run_trade_path_with_direction(
    ...
    if direction == 1:  # Long trade
        ...
        pt_level = entry_price * (
            1 - pt_multiple * entry_volatility
        )  # Profit if price goes down
        sl_level = entry_price * (
            1 + sl_multiple * entry_volatility
...
def validate_trend_strategy_oos(
    prices: pd.Series,
    entry_signal_fn: Callable[
        [pd.Series], pd.Series
    ],  # Function that generates direction signals
...
def calculate_directional_metrics(trades_df: pd.DataFrame) -> Dict:
    ...
    overall_sharpe = (
        overall_returns.mean() / overall_returns.std()
    ...
    return {
        ...
        "mean_return_short": short_trades["return"].mean()
