...
def _get_number_of_backtest_paths(n_train_splits: int, n_test_splits: int) -> float:
    ...
    """
    return int(
        comb(n_train_splits, n_train_splits - n_test_splits) * n_test_splits / n_train_splits
    )
...
class CombinatorialPurgedKFold(_BaseKFold):
    ...
    def _fill_backtest_paths(self, train_indices: list, test_splits: list):
        ...
        for split in test_splits:
            found = (
                False  # Flag indicating that split was found and filled in one of backtest paths
            )
    ...
    def split(self, X: pd.DataFrame, y: pd.Series = None, groups=None):
        ...
        test_ranges = [
            (ix[0], ix[-1] + 1) for ix in np.array_split(np.arange(X.shape[0]), self.n_splits)
        ]
        splits_indices = {}
        ...
        for test_splits in combinatorial_test_ranges:
            ...
            # Get indices
            train_indices = self.samples_info_sets.index.get_indexer(train_times.index).tolist()
