...
def _improve_clusters(
    corr_mat: pd.DataFrame, clusters: dict, top_clusters: dict
) -> Union[pd.DataFrame, dict, pd.Series]:
    """
    ...
    silh_scores_new = pd.Series(
        silhouette_samples(dist, kmeans_labels), index=dist.index
    )
    return corr_new, clusters_new, silh_scores_new


def _cluster_kmeans_base(
    ...
    if not is_distance_matrix:
        # If the input is a correlation matrix, convert it to a distance matrix
        corr_mat.clip(-1, 1, inplace=True)  # Sanitize for float precision errors
        np.fill_diagonal(corr_mat.values, 1)
        distance = ((1 - corr_mat.fillna(0)) / 2.0) ** 0.5
    else:
        # If the input is already a distance matrix, use it directly
        distance = corr_mat
    ...
    for _ in range(repeat):
        for num_clusters in range(2, max_num_clusters + 1):
            kmeans_ = KMeans(n_clusters=num_clusters, n_init="auto")
    ...
    # Cluster members
    clusters = {
        i: corr_mat.columns[np.where(kmeans.labels_ == i)[0]].tolist()
...
def cluster_kmeans_top(
    corr_mat: pd.DataFrame, repeat: int = 10, is_distance_matrix: bool = False
) -> Union[pd.DataFrame, dict, pd.Series, bool]:
    """
    ...
    max_num_clusters = (
        min(corr_mat.drop_duplicates().shape[0], corr_mat.drop_duplicates().shape[1])
        - 1
    )
    corr1, clusters, silh = _cluster_kmeans_base(
        corr_mat,
    ...
    cluster_quality = {
        i: (
            ...
            else np.mean(silh[clusters[i]]) / np.std(silh[clusters[i]])
        )
        for i in clusters.keys()
    }
    avg_quality = np.mean(list(cluster_quality.values()))
    redo_clusters = [
        i for i in cluster_quality.keys() if cluster_quality[i] < avg_quality
    ]
    ...
    if len(redo_clusters) <= 2:
        ...
        corr_new, clusters_new, silh_new = _improve_clusters(
            corr_mat,
        ...
        return _check_improve_clusters(
            new_tstat_mean,
            mean_redo_tstat,
            (corr1, clusters, silh),
...
def get_onc_clusters(
    ...
    :param repeat: (int) Number of clustering algorithm repetitions
    :param is_distance_matrix: (bool) If False, the correlation matrix is transformed to a matrix of distances
    :return: (tuple) [correlation matrix, optimized clusters, silh scores]
    """

    return cluster_kmeans_top(corr_mat, repeat, is_distance_matrix)
