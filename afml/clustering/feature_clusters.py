...
def get_feature_clusters(
    ...
    :param distance_metric: (str) The distance operator to be used for generating the distance matrix. The methods that
                            ...
                            features are to be generated as is by the ONC algorithm.
    :param linkage_method: (str) Method of linkage to be used for clustering. Methods include: 'single', 'ward',
                           ...
                           be generated as is by the ONC algorithm.
    ...
    :param critical_threshold: (float) Threshold for determining low silhouette score in the dataset. It can any real number
                                ...
                                indentified as having low silhouette and hence requied transformation will be applied to for
    ...
    # Define which metrics are already distance-based
    distance_based_metrics = ["information_variation", "mutual_information"]
    is_distance = dependence_metric in distance_based_metrics
    ...
    if n_clusters is None and (distance_metric is None or linkage_method is None):
        return list(
            get_onc_clusters(dep_matrix.fillna(0), is_distance_matrix=is_distance)[
                1
            ].values()
    ...
    if distance_metric is not None and (
        ...
        n_clusters = len(
            get_onc_clusters(dep_matrix.fillna(0), is_distance_matrix=is_distance)[1]
        )
    if n_clusters >= len(
        X.columns
...
def _check_for_low_silhouette_scores(
    ...
    Checks where the dataset contains features low silhouette due to one feature being a combination of
    ...
    print(
        "No feature/s found with low silhouette score. All features belong to their own respective clusters."
    )
