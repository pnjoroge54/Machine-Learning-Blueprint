...
from ..cross_validation.optuna_hyper_fit import optimize_trading_model, optuna_to_cv_results
...
class ModelDevelopmentPipelineOptuna:
    """
    ...
    def train_model(self):
        ...
        y = self.events["bin"]
        study = optimize_trading_model(
          X,
          y,
          events=self.events,
          data_index=self.bar_data.index,
          base_model_instance=pipe,
          param_distributions=self.model_params['param_grid'],
          # sample_weight=self.events["w"],
        )

        self.best_model = set_pipeline_params(pipe, **study.best_trial.params)
        self.cv_results = optuna_to_cv_results(study)
