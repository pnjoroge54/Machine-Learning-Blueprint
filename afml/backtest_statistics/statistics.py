...
def timing_of_flattening_and_flips(target_positions: pd.Series) -> pd.DatetimeIndex:
    ...
    Flattenings - times when open position is being closed (final target position is 0).
    ...
    multiplied_positions = target_positions.iloc[1:] * target_positions.iloc[:-1].values
...
def average_holding_period_numba(target_positions, position_difference, time_difference):
    ...
    for i in range(1, target_positions.size):
        ...
        if (position_difference[i] * target_positions[i - 1]) >= 0:
            if (target_positions[i]) != 0:  # And not an empty position
                entry_time = (
                    entry_time * target_positions[i - 1]
                    + time_difference[i] * position_difference[i]
                ) / target_positions[i]

        # Decreased
        else:
            holding_time[i] = time_difference[i] - entry_time

            # Flip of a position
            if (target_positions[i] * target_positions[i - 1]) < 0:
                holding_weight[i] = abs(target_positions[i - 1])
                entry_time = time_difference[i]  # Reset entry time

            # Only a part of position is closed
            else:
                holding_weight[i] = abs(position_difference[i])

    return holding_time, holding_weight
...
def drawdown_and_time_under_water(returns: pd.Series, dollars: bool = False) -> tuple:
    ...
    - Time under water index is the time of a high watermark and how much time
      passed till the next high watermark in days. Also includes time between
    ...
    time_under_water = np.append(
        time_under_water,
        (returns.index[-1] - high_watermarks.index[-1]) / np.timedelta64(1, "D"),
    )

    time_under_water = pd.Series(
        time_under_water, index=high_watermarks.index, name="days_under_water"
    )
...
def deflated_sharpe_ratio(
    ...
    if estimates_param:
        benchmark_sr = sr_estimates[0] * (
            (1 - np.euler_gamma) * norm.ppf(1 - 1 / sr_estimates[1])
            + np.euler_gamma * norm.ppf(1 - 1 / sr_estimates[1] * np.e ** (-1))
        )

    # Calculating benchmark_SR from a list of estimates
    else:
        benchmark_sr = np.array(sr_estimates).std() * (
            (1 - np.euler_gamma) * norm.ppf(1 - 1 / len(sr_estimates))
            + np.euler_gamma * norm.ppf(1 - 1 / len(sr_estimates) * np.e ** (-1))
        )
