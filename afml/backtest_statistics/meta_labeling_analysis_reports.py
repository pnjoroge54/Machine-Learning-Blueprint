...
from .meta_labeling_analysis import analyze_signal_quality, calculate_risk_adjusted_metrics
...
def compare_strategies(results: dict, verbose: bool = True) -> pd.DataFrame:
    ...
    if verbose:
        ...
        print(f"  Bet Sizing:        {results['bet_sizing']}")
        print(f"  Confidence Thresh: {results['confidence_threshold']:.2f}\n")
...
def generate_summary_report(results: dict) -> str:
    ...
    report.append(f"Confidence Threshold:     {results['confidence_threshold']:.2f}")
    report.append("")
        ...
        signal_table = pd.DataFrame(
            {
                ...
                "Value": [
                    ...
                    f"{signal_analysis.get('filter_rate', 0):.2%}",
                    f"{signal_analysis.get('accepted_win_rate', 0):.2%}",
                    f"{signal_analysis.get('rejected_win_rate', 0):.2%}",
                    f"{signal_analysis.get('avoided_losses', 0):.2%}",
                    f"{signal_analysis.get('precision', 0):.2%}",
                    f"{signal_analysis.get('recall', 0):.2%}",
                    f"{signal_analysis.get('f1_score', 0):.2%}",
                ],
            }
        )
