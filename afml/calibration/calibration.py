...
def plot_reliability(
    ...
    if draw_hist:
        ...
        hist_ax.hist(p_pred, bins=20, range=(0, 1), color='lightblue', 
...
def plot_reliability_with_ci(
    ...
    base_df, lower, upper = bootstrap_reliability_ci(
        y_true, p_pred, n_bins=n_bins, n_bootstraps=n_bootstraps,
    ...
    ax.plot(base_df.loc[valid_mask, 'pred_mean'], 
            base_df.loc[valid_mask, 'true_frac'], 
            'o-', color='blue', linewidth=2, markersize=6, 
            label='Calibration Curve')
    
    ax.fill_between(base_df.loc[valid_mask, 'pred_mean'], 
                   lower[valid_mask], upper[valid_mask], 
...
def apply_calibration(
    calibrator: Union[LogisticRegression, IsotonicRegression], 
    scores: np.ndarray
) -> np.ndarray:
    """
...
def calibration_report(
    ...
    return pd.DataFrame([metrics]).T.rename(columns={0: 'value'})    return pd.DataFrame([metrics]).T.rename(columns={0: 'value'})