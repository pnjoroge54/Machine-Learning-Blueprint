...
class CVIsotonicCalibrator(BaseEstimator, ClassifierMixin):
    """
    Probability calibrator using isotonic regression fitted on out-of-fold
    (OOF) predictions generated by PurgedKFold cross-validation.

    Standard sklearn calibration methods (``CalibratedClassifierCV``) use
    either a random held-out split or stratified k-fold to produce the
    predictions on which the isotonic map is fitted. Both approaches violate
    temporal ordering and the embargo requirement that prevents label
    information from leaking across the purge boundary. This class corrects
    that by generating OOF predictions through ``PurgedKFold``, which purges
    overlapping labels and applies a configurable embargo before each test
    fold.
    ...
    2. **Isotonic calibration** — ``IsotonicRegression`` with
       ``out_of_bounds='clip'`` is fitted on the OOF probabilities and true
       labels, weighted by ``sample_weight`` when provided. This produces a
       monotone non-decreasing map from raw probability to calibrated
       probability.
    3. **Full-data refit** — the base estimator is cloned again and refitted
       on the complete training dataset (all folds combined), so that
       ``predict_proba()`` at inference time uses a model with maximum
       historical coverage while applying the calibration map learned from
       the OOF predictions.

    Parameters
    ----------
    estimator : estimator object
        An unfitted sklearn-compatible classifier that implements
        ``predict_proba``. The estimator is cloned at each fold to prevent
        state leakage between folds. A ``Pipeline`` ending in a classifier
        is fully supported.

    cv : PurgedKFold instance, required
        Cross-validation strategy. A pre-configured ``PurgedKFold`` instance
        must be supplied explicitly. This requirement is intentional: there is
        no financially sensible default because ``PurgedKFold`` requires a
        ``t1`` Series (label end times) that is specific to each dataset.
        Omitting ``cv`` raises ``NotImplementedError`` at ``fit()`` time with
        an explanatory message. Using ``StratifiedKFold`` or any non-purging
        splitter will silently leak label information across fold boundaries
        and produce optimistically biased calibration.
    ...
    calibrator_ : IsotonicRegression
        The fitted ``IsotonicRegression`` instance (Phase 2). Maps raw
        probabilities output by ``estimator_`` to calibrated probabilities.
        Call ``calibrator_.predict(raw_probs)`` directly to apply the
        isotonic map without going through ``predict_proba``.
    ...
    n_features_in_ : int
        Number of features seen during ``fit``. Conforms to the sklearn
        estimator API convention used by ``check_is_fitted`` and
        pipeline compatibility checks.

    Examples
    --------
    Basic usage with a pre-configured ``PurgedKFold``:

    >>> from afml.calibration.calibration import CVIsotonicCalibrator
    >>> from afml.cross_validation.cross_validation import PurgedKFold
    >>> from sklearn.ensemble import RandomForestClassifier
    >>>
    >>> cv = PurgedKFold(n_splits=5, t1=events['t1'], pct_embargo=0.01)
    >>> clf = CVIsotonicCalibrator(
    ...
    >>>
    >>> # Calibrated probabilities at inference time
    ...
    >>>
    >>> # Apply the isotonic map to an externally generated probability array
    >>> raw_scores = clf.estimator_.predict_proba(X_new)[:, 1]
    >>> calibrated = clf.calibrator_.predict(raw_scores)

    Integration with the ModelDevelopmentPipeline:

    >>> model, features, metrics, config = pipeline.run(calibrate=True)
    >>> # pipeline.calibrator_ is the fitted CVIsotonicCalibrator
    >>> # pipeline.best_model is the same object — predict_proba returns
    >>> # calibrated probabilities directly.

    Notes
    -----
    **Scoring convention** — the default ``score()`` method returns
    **Probability-Weighted Accuracy (PWA)**, not plain accuracy. PWA rewards
    confident correct predictions and penalizes confident incorrect ones,
    making it the appropriate default for a calibrator whose primary purpose
    is producing well-formed probabilities. Use ``brier_score()`` for the
    classical calibration quality metric (mean squared probability error,
    lower is better).

    **Sample weights** are passed to both the base estimator's ``fit()`` call
    (if it accepts ``sample_weight``) and to ``IsotonicRegression.fit()``.
    Passing the same AFML-style uniqueness or time-decay weights used for
    model training ensures the calibration curve is fitted on the same
    information weighting as the model itself, maintaining internal
    consistency.

    **ONNX export** — ``CVIsotonicCalibrator`` has no ONNX operator mapping.
    When exporting to ONNX for deployment in MetaTrader 5, use
    ``estimator_`` (the inner sklearn model) as the ONNX source and apply
    ``calibrator_.predict()`` as a separate post-processing step on the
    ONNX model's raw output probabilities.

    **Re-estimation** — the isotonic map reflects the miscalibration
    structure of the model on the training data distribution. After a
    significant regime shift or after retraining the base estimator on a
    substantially different dataset, call ``fit()`` again to update the
    map. Stale calibration is preferable to no calibration, but the map
    degrades as the training distribution drifts away from the deployment
    distribution.

    **Minimum calibration data** — isotonic regression requires enough
    OOF observations for the step function to be well-estimated. If the
    effective sample size (after purging and embargo) is fewer than
    approximately 200 observations, consider using Platt scaling
    (``fit_platt_scaling``) instead, which has only two parameters and
    is more stable on small calibration sets.
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        2. Fit IsotonicRegression on OOF probabilities, true labels, and
           sample weights.
        ...
        sample_weight : array-like of shape (n_samples,), optional
            ...
            to the base estimator's ``fit()`` (if it accepts
            ``sample_weight``) and to ``IsotonicRegression.fit()``.
            If ``None``, all observations are weighted equally.

        Returns
        -------
        self : CVIsotonicCalibrator
            Fitted calibrator.

        Raises
        ------
        NotImplementedError
            If ``cv`` was not supplied at construction time. A PurgedKFold
            instance with the appropriate ``t1`` Series must be passed
            explicitly — there is no financially sensible default.

        ValueError
            If fewer than 10 valid OOF predictions are produced after
            purging and embargo. This typically indicates that the embargo
            fraction is too large relative to the dataset size, or that
            ``n_splits`` is set too high for the available observations.
        """
        ...
        for fold_idx, (train_idx, test_idx) in enumerate(self.cv_.split(X, y)):
    ...
    def predict_proba(self, X):
        ...
        Applies the full two-step inference chain:
        ``X → estimator_.predict_proba(X) → calibrator_.predict(raw) → [1-p, p]``
        ...
        proba : ndarray of shape (n_samples, 2)
            ...
            class. Values are clipped to [0, 1] by the ``IsotonicRegression``
            ``out_of_bounds='clip'`` setting.
    ...
    def score(self, X, y, sample_weight=None):
        ...
        incorrect ones, making it the appropriate default score for a
        calibrator whose purpose is producing well-formed probabilities.
        Plain accuracy discards the probability estimates entirely; PWA
        evaluates them.
    ...
    def brier_score(self, X, y, sample_weight=None):
        ...
        probabilities and true binary outcomes. It is the canonical metric
        for evaluating probability calibration quality: a perfectly calibrated
        model achieves a Brier score equal to the base rate variance of the
        label distribution.
